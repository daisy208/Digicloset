  ...
  const exportImage = async () => {
    ...
    try {
      if (userPhoto && selectedItems.length > 0) {
        ...
        const result = await aiService.processVirtualTryOn(userPhoto, selectedItems, lightingSettings);
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden w-full max-w-6xl mx-auto">
      ...
      <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0">
          <button
            ...
            className="flex-1 flex items-center justify-center space-x-1 px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isProcessing ? <RotateCcw size={16} className="animate-spin" /> : <Download size={16} />}
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center space-x-1 px-3 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Share2 size={16} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
        ...
        <div className="aspect-[3/4] bg-gray-100 relative overflow-hidden rounded-xl">
          {userPhoto ? (
            <>
              <img src={userPhoto} alt="User" className="w-full h-full object-cover" style={getLightingStyle()} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {fitAnalysis && (
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-xs text-gray-600">Recommended: {fitAnalysis.size_recommendation}</p>
              </div>
            )}
          </div>
      ...
      <div className="px-4 pb-4">
        <div className="bg-gray-50 rounded-lg p-3">
          <p className="text-xs text-gray-600">
            ...
            {lightingSettings.scenario.charAt(0).toUpperCase() + lightingSettings.scenario.slice(1)} • Brightness:{' '}
            {lightingSettings.brightness}% • Warmth: {lightingSettings.warmth}%
            {qualityScore && <span className="ml-2">• Quality: {Math.round(qualityScore)}%</span>}
          </p>
        </div>
      </div>
    </div>
